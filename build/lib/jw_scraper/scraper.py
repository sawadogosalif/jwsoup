...
def scrape_single_page(url: str) -> tuple:
    ...
    if next_page:
        ...
        if next_button:
            ...
            if next_url:
                next_url = urllib.parse.urljoin(url, next_url.split("#")[0])  # Remove fragment if present
...
def save_verses_to_parquet(verses: list, current_url: str, output_dir: str, page_sep: str="books"):
    ...
    verses_with_metadata = [{"verse_id": verse[0], "verse_text": verse[1], "source_url": current_url} for verse in verses]
...
def scrape_multi_page(start_url: str, output_dir="data_JW.parquet", max_pages: int = 10000):
    ...
    while current_url and i < max_pages:
        ...
        if verses:
            # Save verses to parquet
            page = urllib.parse.unquote(current_url.split("/")[-1].strip("/"))
            save_verses_to_parquet(verses, current_url, output_dir, page)
